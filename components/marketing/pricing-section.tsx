    ...
    <section className="px-4 py-20" id="preturi">
      <Container>
        ...
        <ScrollReveal className="mx-auto max-w-2xl">
          <SpotlightCard className="rounded-[1.75rem] border border-accent-emerald/20 bg-surface-graphite/80 p-8 text-center backdrop-blur-sm md:p-10">
            ...
            <Link
              className="accent-button gradient-border-animated mt-6 inline-block rounded-xl px-8 py-4 text-base font-bold"
              href="/signup"
            >
        ...
        <div className="mx-auto mt-10 grid max-w-5xl gap-6 sm:gap-8 md:grid-cols-3">
          {pricingPlans.filter((plan) => plan.name !== "Încearcă Gratis!").map((plan, i) => (
            <ScrollReveal key={plan.name} delay={i * 0.08}>
              <div className="relative h-full">
                {plan.badge ? <div className="absolute -top-3 left-1/2 z-20 -translate-x-1/2 whitespace-nowrap rounded-full bg-accent-emerald px-4 py-1 text-xs font-bold uppercase tracking-wider text-crypto-dark shadow-md">{plan.badge}</div> : null}
              <SpotlightCard
                as="article"
                className={`relative flex h-full flex-col rounded-[1.5rem] p-8 hover-glow-bloom ${plan.highlighted ? "border-2 border-accent-emerald bg-surface-graphite shadow-glow" : "panel"}`}
              >
        ...
        <div className="mt-8 space-y-4 text-center">
          <p className="text-slate-400">Plăți securizate prin Card (Stripe) sau Crypto (USDT)</p>
