...
const EMAIL_TEMPLATES: Record<string, string> = {
  ...
  welcome_signup: `<!DOCTYPE html><html lang="ro"><head><meta charset="UTF-8"><meta name="viewport" content="width=device-width, initial-scale=1.0"></head><body style="margin:0;padding:0;background-color:#06110D;font-family:-apple-system,BlinkMacSystemFont,'Segoe UI',Roboto,sans-serif;"><table role="presentation" width="100%" cellpadding="0" cellspacing="0" style="background-color:#06110D;"><tr><td align="center" style="padding:40px 16px;"><table role="presentation" width="100%" cellpadding="0" cellspacing="0" style="max-width:520px;background-color:#0D1F18;border-radius:16px;border:1px solid rgba(11,102,35,0.25);"><tr><td align="center" style="padding:32px 32px 0;"><img src="https://pub-36a9a370a5804b06b1f9c6ab94b83f65.r2.dev/email/armata-mascot.gif" alt="Armata de Traderi" width="180" style="display:block;border-radius:12px;margin:0 auto;" /></td></tr><tr><td align="center" style="padding:20px 32px 16px;"><div style="font-size:24px;font-weight:800;color:#0B6623;">🪖 Armata de Traderi</div></td></tr><tr><td style="padding:0 32px 32px;"><h1 style="margin:0 0 20px;font-size:22px;font-weight:700;color:#FFFFFF;text-align:center;">Bine ai venit în Armata</h1><p style="margin:0 0 24px;font-size:15px;line-height:1.8;color:#A3B8B0;">Contul tău e creat. Mă bucur că ești aici. Înainte să-ți zic ce urmează, vreau să știi un lucru: <strong style="color:#FFFFFF;">ai 7 zile de acces complet la Elite, pe gratis</strong>.</p><p style="margin:0 0 28px;font-size:15px;line-height:1.8;color:#A3B8B0;">Fără card, fără abonament automat. Activezi trial-ul cu un singur click.</p><div style="background:#0A1712;border-radius:12px;padding:16px 20px;margin:0 0 12px;"><p style="margin:0;font-size:14px;color:#FFFFFF;font-weight:600;">Ce intră în Elite</p><p style="margin:6px 0 0;font-size:13px;line-height:1.7;color:#A3B8B0;">Discord privat cu analize live, 67+ video-uri educaționale, portofoliu stocks cu zone Buy/Sell, indicatori TradingView, dashboard whale tracker, calendar economic și risk score zilnic.</p></div><table role="presentation" width="100%" cellpadding="0" cellspacing="0" style="margin-top:24px;"><tr><td align="center"><a href="https://app.armatadetraderi.com/dashboard" style="display:inline-block;padding:14px 40px;background-color:#0B6623;color:#FFFFFF;font-size:16px;font-weight:700;text-decoration:none;border-radius:10px;">Activează Trial Gratuit</a></td></tr></table><p style="margin:20px 0 0;font-size:13px;color:#5A7168;text-align:center;">Întrebări? Răspund personal la contact@armatadetraderi.com.</p></td></tr><tr><td style="padding:24px 32px;border-top:1px solid rgba(11,102,35,0.15);"><p style="margin:0;font-size:12px;color:#5A7168;text-align:center;">Alex Costea - Armata de Traderi<br>armatadetraderi.com</p></td></tr></table></td></tr></table></body></html>`,
...
export async function GET(request: NextRequest) {
  try {
    ...
    for (const email of pendingEmails) {
      ...
      if (!templateHtml) {
        console.error(
          `[cron/send-emails] UNKNOWN TEMPLATE: id=${email.id} email=${email.email} template="${email.template}" — skipped. Known: ${Object.keys(EMAIL_TEMPLATES).join(", ")}`,
        );
        await supabase
          .from("email_drip_queue")
          .update({ status: "skipped", error: `unknown template: ${email.template}` })
          .eq("id", email.id);
        continue;
      }
      ...
      try {
        ...
        const msg = (sendError as Error).message ?? String(sendError);
        console.error(`[cron/send-emails] Resend failed: id=${email.id} to=${email.email} template=${email.template} err=${msg}`);
        await supabase
          .from("email_drip_queue")
