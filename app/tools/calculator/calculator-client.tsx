...
const CURRENCIES = ["EUR", "USDT", "USD"] as const;
...
function formatCurrency(n: number, c: Currency): string {
  if (!Number.isFinite(n)) return "—";
  const symbol = CURRENCY_SYMBOL[c];
  const v = n.toLocaleString("ro-RO", { maximumFractionDigits: 2, minimumFractionDigits: 2 });
  return `${symbol}${v}`;
}
...
function formatUnits(n: number): string {
  ...
  if (n < 1) return n.toLocaleString("ro-RO", { maximumFractionDigits: 6 });
  if (n < 100) return n.toLocaleString("ro-RO", { maximumFractionDigits: 4 });
...
export function CalculatorClient({ isAuthed }: { isAuthed: boolean }) {
  ...
  const result = useMemo(() => {
    ...
    // Direction validation: long needs stop below entry; short needs stop above
    ...
    if (Number.isFinite(t) && t > 0 && t !== e) {
      ...
      // Sanity: long needs target above entry; short needs target below
  ...
  }, [account, riskPct, entry, stop, target, direction]);
          ...
          <div>
            ...
            <div className="mt-2 flex gap-2">
              ...
              <select
                ...
                className="rounded-xl border border-white/10 bg-white/[0.02] px-3 text-sm text-slate-200"
              >
                {CURRENCIES.map((c) => (
                  <option key={c} value={c} className="bg-crypto-dark">
                    {c}
                  </option>
                ))}
              </select>
          ...
          <div>
            ...
            {result?.wrongDirection ? (
              <p className="mt-1 text-xs text-amber-400">
                ⚠ Pe {direction === "long" ? "long" : "short"}, stop-ul ar trebui să fie {direction === "long" ? "sub" : "deasupra"} prețului de intrare.
              </p>
            ) : null}
          </div>
      ...
      <section className="glass-card rounded-2xl border border-emerald-500/20 bg-emerald-500/[0.02] p-5 sm:p-6">
        ...
        {!result ? (
          ...
          <div className="space-y-4">
            <Metric
              label="Sumă riscată"
              value={formatCurrency(result.riskAmount, currency)}
              hint={`${riskPct}% din ${formatCurrency(num(account), currency)}`}
              tone="risk"
            />
            <Metric
              label="Mărime poziție (în unități)"
            ...
            <Metric
              ...
              hint={`${result.positionPctOfAccount.toFixed(1)}% din cont (fără levier)`}
            />
            {result.rrRatio != null && result.rewardAmount != null ? (
              <Metric
                label="Risk / Reward"
                value={`1 : ${result.rrRatio.toFixed(2)}`}
                hint={`Profit potențial: ${formatCurrency(result.rewardAmount, currency)}`}
                tone={result.rrRatio >= 2 ? "good" : result.rrRatio >= 1 ? "neutral" : "bad"}
              />
            ) : null}

            {result.positionPctOfAccount > 100 ? (
              <div className="rounded-xl border border-amber-400/30 bg-amber-400/5 p-3 text-xs text-amber-300">
                ℹ Poziția depășește contul → ai nevoie de levier ({(result.positionPctOfAccount / 100).toFixed(1)}x). Atenție: levierul amplifică pierderile, nu doar profitul.
              </div>
            ) : null}
          </div>
        )}
        ...
        <div className="mt-6 rounded-xl border border-white/10 bg-white/[0.02] p-4 text-xs text-slate-400">
          ...
          <p className="leading-relaxed">
            Cu risc de 1% pe trade, ai nevoie de 100 de pierderi consecutive ca să-ți pierzi contul. La 10% per trade, doar 10. Diferența între cei care rămân pe piață și cei care se prăbușesc.
          </p>
        </div>
        ...
        {!isAuthed ? (
          <div className="mt-4 rounded-xl border border-emerald-500/20 bg-emerald-500/[0.04] p-4">
            ...
            <p className="mt-1 text-xs text-slate-400">
              În Elite primești zone exacte de Buy/Sell pe 16 acțiuni + 25 crypto, indicatori TradingView și analize live pe Discord.
            </p>
            <Link
              href="/upgrade"
...
function Metric({
  ...
  const valueColor =
    tone === "good" ? "text-emerald-400"
      ...
      : tone === "risk" ? "text-amber-300"
      : "text-white";
  return (
    <div className="rounded-xl border border-white/10 bg-white/[0.02] p-4">
      <p className="text-[11px] uppercase tracking-wider text-slate-500">{label}</p>
      <p className={`mt-1 font-data text-xl font-bold tabular-nums sm:text-2xl ${valueColor}`}>{value}</p>
