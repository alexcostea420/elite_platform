...
export default async function BotDashboardPage() {
  ...
  // API key expiry: 90 days from wallet creation
  const apiKeyCreated = wallet?.created_at ? new Date(wallet.created_at) : null;
  const apiKeyExpiry = apiKeyCreated ? new Date(apiKeyCreated.getTime() + 90 * 24 * 60 * 60 * 1000) : null;
  const apiKeyDaysLeft = apiKeyExpiry ? Math.max(0, Math.ceil((apiKeyExpiry.getTime() - now.getTime()) / (1000 * 60 * 60 * 24))) : null;
  const apiKeyExpiring = apiKeyDaysLeft !== null && apiKeyDaysLeft <= 14;
    ...
    <>
      ...
      <main className="pb-16 pt-24 md:pt-28">
        <Container>
          ...
          {apiKeyDaysLeft !== null && (
            <section className={`mb-8 animate-fade-in-up stagger-3 rounded-2xl border px-6 py-4 ${apiKeyExpiring ? "border-amber-500/20 bg-amber-500/5" : "border-white/10 bg-white/5"}`}>
